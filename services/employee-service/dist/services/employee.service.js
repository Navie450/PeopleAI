...
class EmployeeService {
    ...
    async listEmployees(query) {
        ...
        // Apply filters
        if (query.search) {
            queryBuilder.andWhere('(employee.first_name ILIKE :search OR employee.last_name ILIKE :search OR employee.work_email ILIKE :search OR employee.employee_id ILIKE :search OR employee.job_title ILIKE :search)', { search: `%${query.search}%` });
        }
        ...
        const employeeResponses = employees.map((emp) => this.mapToListItemResponse(emp));
        const meta = {
            page,
            limit,
            total,
    ...
    async createEmployee(employeeData, createdBy) {
        ...
        if (existingEmployee) {
            throw new errors_1.ConflictError('User already has an employee record');
        }
        ...
        if (emailExists) {
            throw new errors_1.ConflictError('Work email already in use');
        }
        // Validate department if provided
        if (employeeData.department_id) {
            const departmentRepository = database_1.AppDataSource.getRepository(Department_1.Department);
            const department = await departmentRepository.findOne({
                where: { id: employeeData.department_id, deleted_at: (0, typeorm_1.IsNull)() },
            });
            if (!department) {
                throw new errors_1.ValidationError('Invalid department ID');
            }
        }
        // Validate manager if provided
        if (employeeData.manager_id) {
            const manager = await employeeRepository.findOne({
                where: { id: employeeData.manager_id, deleted_at: (0, typeorm_1.IsNull)() },
            });
            if (!manager) {
        ...
        try {
            ...
            logger_1.logger.info('Employee created successfully:', {
            ...
            logger_1.logger.error('Failed to create employee:', error);
            throw error;
        }
    }
    async updateEmployee(employeeId, employeeData, updatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
        if (!employee) {
            throw new errors_1.NotFoundError('Employee not found');
        }
        ...
        if (employeeData.manager_id && employeeData.manager_id !== employee.manager_id) {
            if (employeeData.manager_id === employeeId) {
                throw new errors_1.ValidationError('Employee cannot be their own manager');
            }
            const manager = await employeeRepository.findOne({
                where: { id: employeeData.manager_id, deleted_at: (0, typeorm_1.IsNull)() },
            });
    ...
    async deleteEmployee(employeeId, deletedBy) {
        ...
        employee.employment_status = Employee_1.EmploymentStatus.TERMINATED;
        await employeeRepository.save(employee);
        // Create audit log
        await this.createAuditLog({
            user_id: deletedBy,
    ...
    async bulkUpdateEmployees(bulkData, updatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        let updated = 0;
        const failed = [];
        for (const empId of bulkData.employee_ids) {
            try {
                const employee = await employeeRepository.findOne({
                    where: { id: empId, deleted_at: (0, typeorm_1.IsNull)() },
                });
                if (!employee) {
                    failed.push(empId);
                    continue;
                ...
                logger_1.logger.error(`Failed to update employee ${empId}:`, error);
                failed.push(empId);
            }
        }
        ...
        logger_1.logger.info('Bulk update completed:', { updated, failed: failed.length });
        return { updated, failed };
    }
    async transferEmployee(employeeId, transferData, transferredBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const departmentRepository = database_1.AppDataSource.getRepository(Department_1.Department);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        ...
        const newDepartment = await departmentRepository.findOne({
            where: { id: transferData.new_department_id, deleted_at: (0, typeorm_1.IsNull)() },
        });
        if (!newDepartment) {
            throw new errors_1.ValidationError('Invalid department ID');
        }
        ...
        if (transferData.new_job_title)
            employee.job_title = transferData.new_job_title;
        if (transferData.new_job_level)
            employee.job_level = transferData.new_job_level;
        if (transferData.new_manager_id)
            employee.manager_id = transferData.new_manager_id;
        if (transferData.new_salary)
            employee.base_salary = transferData.new_salary;
        await employeeRepository.save(employee);
        // Create audit log
        await this.createAuditLog({
        ...
        logger_1.logger.info('Employee transferred:', { employeeId, newDepartment: newDepartment.name });
        return this.getEmployeeById(employeeId);
    }
    async promoteEmployee(employeeId, promoteData, promotedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
            relations: ['department'],
        ...
        if (promoteData.new_job_level)
            employee.job_level = promoteData.new_job_level;
        if (promoteData.new_salary)
            employee.base_salary = promoteData.new_salary;
        await employeeRepository.save(employee);
        // Create audit log
        await this.createAuditLog({
            user_id: promotedBy,
        ...
        logger_1.logger.info('Employee promoted:', {
            employeeId,
            from: previousTitle,
    ...
    async terminateEmployee(employeeId, terminateData, terminatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
        ...
        logger_1.logger.info('Employee terminated:', { employeeId });
        return this.getEmployeeById(employeeId);
    }
    ...
    async getOrgChart(rootEmployeeId) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        ...
        const employees = await employeeRepository
            ...
            .leftJoinAndSelect('employee.department', 'department')
            .where('employee.deleted_at IS NULL')
            .andWhere('employee.employment_status = :status', { status: Employee_1.EmploymentStatus.ACTIVE })
            .getMany();
        // Build hierarchy
    ...
    async getAnalytics() {
        ...
        const activeEmployees = await employeeRepository.count({
            where: { deleted_at: (0, typeorm_1.IsNull)(), employment_status: Employee_1.EmploymentStatus.ACTIVE },
        });
        // By status
        ...
        const remoteCount = await employeeRepository.count({
            where: { deleted_at: (0, typeorm_1.IsNull)(), is_remote: true },
        });
        // Average tenure
        ...
        const probationEndingSoon = await employeeRepository
            .createQueryBuilder('employee')
            .where('employee.deleted_at IS NULL')
            .andWhere('employee.employment_status = :status', { status: Employee_1.EmploymentStatus.PROBATION })
    ...
    async searchEmployees(searchTerm, limit = 10) {
        ...
        const employees = await employeeRepository
            ...
            .leftJoinAndSelect('employee.manager', 'manager')
            .where('employee.deleted_at IS NULL')
            .andWhere('(employee.first_name ILIKE :search OR employee.last_name ILIKE :search OR employee.work_email ILIKE :search OR employee.employee_id ILIKE :search)', { search: `%${searchTerm}%` })
            .take(limit)
            .getMany();
        return employees.map((emp) => this.mapToListItemResponse(emp));
    }
    async getEmployeesBySkill(skillName, minLevel) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
    ...
    async updateSkills(employeeId, skills, updatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
    ...
    async updateLeaveBalance(employeeId, leaveType, balanceUpdate, updatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
    ...
    async addPerformanceGoal(employeeId, goal, addedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
    ...
    async updatePerformanceGoal(employeeId, goalId, updates, updatedBy) {
        const employeeRepository = database_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepository.findOne({
            where: { id: employeeId, deleted_at: (0, typeorm_1.IsNull)() },
        });
        ...
        if (goalIndex === -1) {
            throw new errors_1.NotFoundError('Performance goal not found');
        }
        goals[goalIndex] = {
            ...goals[goalIndex],
            ...updates,
    ...
    mapToListItemResponse(employee) {
        return {
            id: employee.id,
            employee_id: employee.employee_id,
