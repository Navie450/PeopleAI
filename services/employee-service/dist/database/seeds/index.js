...
const seed = async () => {
    try {
        logger_1.logger.info('Starting employee-service seed...');
        ...
        if (existingDepts > 0) {
            logger_1.logger.info('Departments already exist, skipping department seed');
        }
        else {
            const departments = [
                {
        ...
        if (existingEmployees > 0) {
            logger_1.logger.info('Employees already exist, skipping employee seed');
        }
        else {
            ...
            const employees = [
                ...
                },
                {
                    ...
                    department_code: 'MKT',
                    employment_type: Employee_1.EmploymentType.FULL_TIME,
                    employment_status: Employee_1.EmploymentStatus.PROBATION,
            ...
            for (const empData of employees) {
                ...
                logger_1.logger.info(`Created employee: ${empData.employee_id} - ${empData.first_name} ${empData.last_name}`);
            }
        }
        // ── Summary ─────────────────────────────────────────────────
        const totalDepts = await departmentRepository.count();
        const totalEmployees = await employeeRepository.count();
        logger_1.logger.info('=== Employee Seed Complete ===');
        logger_1.logger.info(`Departments: ${totalDepts}, Employees: ${totalEmployees}`);
        logger_1.logger.info('Note: Run auth-service seed first, then link users to employees via the admin UI');
        await database_1.AppDataSource.destroy();
        logger_1.logger.info('Database connection closed');
    }
    catch (error) {
        logger_1.logger.error('Error seeding employee database:', error);
        process.exit(1);
    }
};
seed();
